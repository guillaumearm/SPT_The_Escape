...
class Mod implements IPostDBLoadMod {
  public preSptLoad(container: DependencyContainer): void {
    ...
    logger.logWithColor(
      `Database item table state: ${
        databaseServer.getTables().templates
      } (<<< should be undefined)`,
      LogTextColor.RED,
  ...
  public postDBLoad(container: DependencyContainer): void {
    ...
    if (config.TraderChanges.enabled) {
      if (config.TraderChanges.Better_Sales_To_Traders.enabled) {
        if (debug) {
          for (const trader in traderlist) {
            ...
            log('"category": [');
            traderlist[trader].base.items_buy.category.forEach((x) =>
              log(`"${x}", // ${getItemName(x)}`)
            );
            log("],");
            log('"id_list": [');
            traderlist[trader].base.items_buy.id_list.forEach((x) =>
              log(`"${x}", // ${getItemName(x)}`)
            );
            log("]}");
          }
        ...
        if (debug) {
          for (const trader in traderlist) {
            ...
            traderlist[trader].base.sell_category.forEach((x) =>
              log(`"${x}", // ${locales["en"][x]}`)
            );
          ...
          for (const trader in traderlist) {
            log(
              `${traderlist[trader].base.nickname}: ${
                100 - traderlist[trader].base.loyaltyLevels[3].buy_price_coef
              }%`
            );
          }
        }
        ...
        try {
          ...
          peacekeeper.base.loyaltyLevels.forEach(
            (x) => (x.buy_price_coef += 5)
          );
      ...
      if (config.TraderChanges.Reasonably_Priced_Cases.enabled == true) {
        try {
          therapist.assort.barter_scheme["666aa327e8e00edadd0d2497"][0].forEach(
            (x) => (x.count = 5)
          ); // T H I C C item case (LEDX) 5c0a840b86f7742ffa4f2482
          therapist.assort.barter_scheme["666aa327e8e00edadd0d24d3"][0].forEach(
            (x) => (x.count = 10)
          ); // T H I C C item case (Moonshine)5c0a840b86f7742ffa4f2482
          therapist.assort.barter_scheme["666aa326e8e00edadd0d2473"][0].forEach(
            (x) => (x.count = 7256)
          ); // Item case (Euro) 13839 59fb042886f7746c5005a7b2
          therapist.assort.barter_scheme["666aa327e8e00edadd0d249d"][0].forEach(
            (x) => (x.count = 8)
          ); // Item case (OScope) 59fb042886f7746c5005a7b2
          therapist.assort.barter_scheme["666aa327e8e00edadd0d24b2"][0].forEach(
            (x) => (x.count = 25)
          ); // Item case (Dogtags) 59fb042886f7746c5005a7b2
          therapist.assort.barter_scheme["666aa327e8e00edadd0d24af"][0].forEach(
            (x) => (x.count = 20)
          ); // Lucky Scav Junk box (Dogtags) 5b7c710788a4506dec015957
          therapist.assort.barter_scheme["666aa327e8e00edadd0d2494"][0].forEach(
            (x) => (x.count = 961138)
          ); // Lucky Scav Junk box (Rubles) 1106138 5b7c710788a4506dec015957
          therapist.assort.barter_scheme["666aa327e8e00edadd0d249a"][0].forEach(
            (x) => (x.count = 290610)
          ); // Medcase (Rubles) 5aafbcd986f7745e590fff23
          therapist.assort.barter_scheme["666aa328e8e00edadd0d2518"][0].forEach(
            (x) => (x.count /= 25)
          ); // LEDX (Dogtags) // Really BSG? 160 kills for a non-FIR item? REALLY?! 5c0530ee86f774697952d952

          peacekeeper.assort.barter_scheme[
            "6492e44bf4287b13040fca51"
          ][0].forEach((x) => (x.count = Math.round(x.count / 5 + 1))); // THICC case (SMT+Bluefolder+SecureFlashDrive) 5c0a840b86f7742ffa4f2482

          skier.assort.barter_scheme["666aa2e0e8e00edadd0d03f6"][0].forEach(
            (x) => (x.count = 4)
          ); // Weapon case (Moonshine) 59fb023c86f7746d0d4b423c
        } catch (error) {
          logger.warning(
        ...
        try {
          ...
          for (const trader in traderlist) {
            for (const item in traderlist[trader].assort.items) {
              ...
              if (
                traderlist[trader].assort.items[item]?.upd?.BuyRestrictionMax
              ) {
                traderlist[trader].assort.items[item].upd.BuyRestrictionMax *=
                  config.TraderChanges.Bigger_Limits.Limit_Multiplier;
              }
            }
          }
        } catch (error) {
    ...
    if (config.EconomyOptions.Disable_Flea_Market_Completely.disable) {
      ...
      try {
        globals.RagFair.minUserLevel =
          config.EconomyOptions.Fleamarket_Opened_at_Level.value;
      } catch (error) {
        logger.warning(
