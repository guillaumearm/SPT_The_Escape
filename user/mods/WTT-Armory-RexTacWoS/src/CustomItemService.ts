...
export class CustomItemService {
  ...
  public postDBLoad(): void {
    ...
    this.questModifier.modifyQuests(
      this.Instance.database,
      this.Instance.jsonUtil,
      this.Instance.debug
    );
  }
        ...
        if (location.staticLoot) {
          ...
          if (staticLoot.hasOwnProperty(containerID)) {
            ...
            if (lootContainer) {
              ...
              if (this.Instance.debug) {
                console.log(
                  `Added ${itemToAdd} to loot container: ${containerID} in location: ${locationID}`
                );
              }
            } else {
              if (this.Instance.debug) {
                console.log(
                  `Error: Loot container ID ${containerID} not found in location: ${locationID}`
                );
              }
            }
          } else {
          ...
          if (this.Instance.debug) {
            console.warn(
              `Warning: No static loot found in location: ${locationID}`
            );
          }
        }
      }
  ...
  private processStaticLootContainers(itemConfig: any, itemId: string): void {
    if (itemConfig.addtoStaticLootContainers) {
      ...
      if (Array.isArray(itemConfig.StaticLootContainers)) {
        ...
        if (this.Instance.debug) {
          console.log(
            `Added to container '${staticLootContainer}' with probability ${itemConfig.Probability}`
          );
        }
      }
    }
          ...
          for (const modSlot of parentItem._props.Slots) {
            if (lowercaseModSlots.includes(modSlot._name?.toLowerCase())) {
              ...
              if (!modSlot._props.filters[0].Filter.includes(itemId)) {
                ...
                if (this.Instance.debug) {
                  console.log(
                    `Successfully added item ${itemId} to the filter of mod slot ${modSlot._name} for parent item ${parentItemId}`
                  );
                }
              }
            }
  ...
  private processInventorySlots(
    ...
    if (itemConfig.addtoInventorySlots) {
      ...
      for (const slot of defaultInventorySlots) {
        ...
        const slotId =
          Object.keys(inventorySlots).find(
        ...
        if (
          ...
          if (!slot._props.filters[0].Filter.includes(itemId)) {
            ...
            if (this.Instance.debug) {
              console.log(
                `Successfully added item ${itemId} to the filter of slot ${slot._name}`
              );
            }
          }
        }
    ...
    if (itemConfig.masteries) {
      ...
      for (const mastery of masterySections) {
        ...
        if (existingMastery) {
          ...
          if (this.Instance.debug) {
            console.log(
              ` - Adding to existing mastery section for item: ${itemId}`
            );
          }
        } else {
   ...
   * @return {void} This function does not return a value.
   */
  private processTraders(itemConfig: ConfigItem[string], itemId: string): void {
    const tables = this.Instance.database;
    ...
    for (const item of traderItems) {
      ...
      if (this.Instance.debug) {
        console.log(
          `Successfully added item ${itemId} to the trader ${traderId}`
        );
      }
    }
    ...
    for (const scheme of barterScheme) {
      ...
      if (this.Instance.debug) {
        console.log(
          `Successfully added item ${itemId} to the barter scheme of trader ${traderId}`
        );
      }
    }
  ...
  private addtoHallofFame(itemConfig: ConfigItem[string], itemId: string) {
    const hallofFame1 =
      this.Instance.database.templates.items["63dbd45917fff4dee40fe16e"];
    const hallofFame2 =
      this.Instance.database.templates.items["65424185a57eea37ed6562e9"];
    const hallofFame3 =
      this.Instance.database.templates.items["6542435ea57eea37ed6562f0"];
        ...
        for (const slot of hall._props.Slots) {
          for (const filter of slot._props.filters) {
            if (!filter.Filter.includes(itemId)) {
              ...
              if (this.Instance.debug) {
                console.log(
                  `Added item ${itemId} to filter Hall of Fame ${hall._name}`
                );
              }
            }
          }
  ...
  private processInventoryType(
    ...
    if (
      typeLabel === "equipment" &&
      ((inventoryType.FirstPrimaryWeapon &&
        inventoryType.FirstPrimaryWeapon[finalTplToClone]) ||
        (inventoryType.SecondPrimaryWeapon &&
      ...
      if (!this.ensureValidWeaponPreset(itemId)) {
        ...
        this.processAmmoAndChambers(
          tables.bots.types[botType].inventory,
          tables.templates.items[itemId]._props,
          itemId,
          botType
        );
      }
    ...
    for (const lootSlot in inventoryType) {
      ...
      if (items && items[finalTplToClone] !== undefined) {
        ...
        if (this.Instance.debug) {
          console.log(
            ` - Adding item to bot ${typeLabel} for bot type: ${botType} in loot slot: ${lootSlot} with weight: ${weight}`
          );
        }
        items[itemId] = weight;
          ...
          for (const filter of filters) {
            for (const modId of filter.Filter) {
              if (modId && tables.templates.items[modId]) {
                tables.bots.types[botType].inventory.mods[itemId] =
                  tables.bots.types[botType].inventory.mods[itemId] || {};
                tables.bots.types[botType].inventory.mods[itemId][slotName] =
                  tables.bots.types[botType].inventory.mods[itemId][slotName] ||
                  [];
                if (
                  !tables.bots.types[botType].inventory.mods[itemId][
                ...
                if (this.Instance.debug) {
                  console.log(
                    ` - Added mod ${modId} to ${itemId}'s ${slotName} of bot type ${botType}`
                  );
                }
              }
            }
  ...
  private processBotModSlots(
    ...
    for (const item in mods) {
      ...
      for (const modSlot of modSlots) {
        if (
          itemMods[modSlot] &&
          itemMods[modSlot].includes(finalItemTplToClone)
        ) {
          itemMods[modSlot].push(itemId);
          if (this.Instance.debug) {
            console.log(
              ` - Added item ${itemId} to mod slot ${modSlot} for bot type ${botType} in item ${item}`
            );
          }
  ...
  private processAmmoAndChambers(
    ...
    if (this.Instance.debug) {
      console.log(
        ` - Added new caliber ${ammoCaliber} to bot inventory for bot type ${botType}`
      );
    }
        ...
        if (filters && filters.length > 0) {
          for (const filter of filters) {
            for (const filterItem of filter.Filter) {
              botInventory.Ammo[ammoCaliber][filterItem] =
                botInventory.Ammo[ammoCaliber][filterItem] || 0;
              if (this.Instance.debug) {
                console.log(
                  ` - Added filter item ${filterItem} to caliber ${ammoCaliber} in bot inventory for bot type ${botType}`
                );
              }
            }
          }
  ...
  private ensureValidWeaponPreset(itemId: string): boolean {
    ...
    if (this.Instance.debug) {
      console.warn(
        `No valid preset found for item ${itemId} in globals.ItemPresets`
      );
    }
    return false;
  }
